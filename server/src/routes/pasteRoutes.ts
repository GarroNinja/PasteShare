  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error('Transaction rolled back due to error:', (error as Error).message);
      } catch (rollbackError) {
    ...
    console.error('Unhandled error in update paste route:', (error as Error).message);
    return res.status(500).json({
      message: 'Server error updating paste',
      error: (error as Error).message,
